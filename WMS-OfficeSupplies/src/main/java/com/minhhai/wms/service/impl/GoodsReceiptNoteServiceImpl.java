...
public class GoodsReceiptNoteServiceImpl implements GoodsReceiptNoteService {
    ...
    private String postGRNForPO(GoodsReceiptNote grn, Map<Integer, Integer> receivedQtyMap) {
        ...
        for (GoodsReceiptDetail grnDetail : grn.getDetails()) {
            ...
            if (inputReceivedQty > 0) {
                ...
                // [RACE CONDITION FIX]: Lost Update (Ghi đè số lượng)
                // Khóa dòng Batch này để đảm bảo khi nhiều lệnh Post GRN cùng xử lý,
                // hệ thống sẽ tuần tự cộng QtyAvailable, không bị mất số lượng.
                StockBatch stockBatch = stockBatchDao.findByWarehouseIdAndProductIdAndBinIdAndBatchNumberForUpdate(
    ...
    private String postGRNForTO(GoodsReceiptNote grn, Map<Integer, Integer> receivedQtyMap) {
        ...
        for (GoodsReceiptDetail grnDetail : grn.getDetails()) {
            ...
            if (input > 0) {
                ...
                // [RACE CONDITION FIX & REFACTOR]: Lost Update + Performance Optimization
                // Lấy StockBatch kèm theo ForUpdate dựa đúng vào BinId thay vì dùng ArrayList và chạy Stream for loop trên RAM.
                StockBatch transitBatch = stockBatchDao
                        .findByWarehouseIdAndProductIdAndBinIdAndBatchNumberForUpdate(
                                warehouse.getWarehouseId(), product.getProductId(),
                                grnDetail.getBin().getBinId(), grnDetail.getBatchNumber())
                        .orElseThrow(() -> new IllegalArgumentException("No in-transit stock found"));

                if (transitBatch.getQtyInTransit() < baseQty) {
                    throw new IllegalArgumentException("Khong du ton kho dang In-transit.");
                }
    ...
    private Bin allocateBin(Integer warehouseId, BigDecimal incomingWeight, String productName) {
        ...
        // Dùng ForUpdate để chặn 2 luồng tự sinh BackOrder tạo ra tranh chấp sức chứa Bin.
        List<Bin> activeBins = binDao.findByWarehouseIdAndIsActiveForUpdate(warehouseId, true);
